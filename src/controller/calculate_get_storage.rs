...
fn calculate_proof(trie_proof: &serde_json::Value, storage_keys: Vec<String>) -> Proof {
    let eth_trie_proofs = serde_json::from_value::<EthTrieProofs>(trie_proof.to_owned()).unwrap();
    ...
    let state_root: U256 = U256::from_str(&storage_keys[0]).unwrap();
    ...
    let eth_storage_proof = &eth_trie_proofs.storage_proof[0];
